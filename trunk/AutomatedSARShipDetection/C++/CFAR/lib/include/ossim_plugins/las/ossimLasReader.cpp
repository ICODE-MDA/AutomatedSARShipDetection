...
{
   static const char M[] = "ossimLas::open";
   if (traceDebug())
   {
   ...
   if ( checkSignature() )
   {
      ...
      if ( m_hdr->getPointDataFormatId() == 1 )
      {
         result = init();
...
void ossimLasReader::getEntryList(std::vector<ossim_uint32>& entryList)const
{
   if ( isOpen() )
   {
      for ( ossim_uint32 entry = 0; entry < 5; ++entry )
      {
...
{
   static const char M[] = "ossimLas::parseVarRecords";
...
{
   static const char M[] = "ossimLas::initFromExternalMetadata";
   if (traceDebug()) ossimNotify(ossimNotifyLevel_DEBUG) << M << " entered...\n";
...
bool ossimLasReader::checkSignature()
{
   bool result = false;
   if ( m_str.is_open() )
   {
